...
export default function BossBattle({ questions, world, worldAccent, onComplete, playNarration }) {
  ...
  const boss = world?.boss ?? { name: 'Scale Boss', emoji: '⚖️', reward: 'Boss Badge 👑' };
  ...
  const handleDismiss = () => {
    ...
    const isCorrect = selected === question?.correctAnswer;
    const newLives = lives - (isCorrect ? 0 : 0); // already decremented above
    ...
    return (
      <div className="boss-banner glass-card">
        ...
        <div className="conversion-fact-box" style={{ marginBottom: 20 }}>
          🔑 Remember: <span className="mass-value">1 kg = 1000 g</span>
        </div>
  ...
  return (
    <div style={{ position: 'relative', width: '100%' }}>
      ...
      <FeedbackOverlay
        ...
        message={selected === question?.correctAnswer ? "That's exactly right! Well done! 🎉" : "Not quite! Remember: 1 kg = 1000 g"}
