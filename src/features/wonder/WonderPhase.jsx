...
export default function WonderPhase({ onComplete, playNarration }) {
  ...
  useEffect(() => {
    if (step >= 2 && !narrationFiredRef.current) {
      ...
      const segments = wonderHookNarration(wonder.id);
      if (segments.length > 0) playNarration?.(segments);
    }
  }, [step, wonder.id, playNarration]);
      ...
      {step >= 3 && (
        ...
        >
          <Button
            ...
            onClick={onComplete}
            style={{
              background: 'linear-gradient(135deg, #7c5cbf, #4f3a91)',
              boxShadow: '0 0 28px rgba(124,92,191,0.5)',
            }}
          >
            Let's Discover! ✨
          </Button>
